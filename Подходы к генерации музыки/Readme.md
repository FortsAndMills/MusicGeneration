...
Итак, как я это понял: пусть есть множество входов x. Положим:
a = sigmoid(Wx + b) - "кодировщик";
x' = sigmoid(W^Tx + b') - "декодировщик";
b' и b отличаются, а вот разницу между x' и x, как и в кодировщике, хочется сделать поменьше. При этом в оригинале все входы-выходы считаются булевыми, и полагается, что нам задано распределение, поэтому обучение параметров W, b, b' про KL-дивергенции между x' и x.

Теперь, при помощи обученной RBM, можно деать такой фокус: сэмплируем рандомно (из равномерного) x0. Преобразуем в a0 по первой формуле, сэмплируем; преобразуем в x1, сэмплируем; и так много-много раз. Такой алгоритм - ни что иное, как схема Гиббса для сэмплирования из распределения с известными вероятностями p(x | a) и p(a | x).

Для музыки это позволило бы генерировать произвольные "аккорды", но вот в чём беда - нужна и какая-то модель обработки последовательности, с памятью желательно, на выходе которой будет... получается, RBM? Какъ это?

RNN-RBM: если я верно понял, LSTM даёт на выходе b'. Мол, таким образом на выходе получается RBM из некоторого семейства, параметрами которого являются W и b (но не b'), и которые обучаются как для обычного RBM.
