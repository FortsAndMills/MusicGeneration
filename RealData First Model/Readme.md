...
В результате последующих избиений стенки головой стенка, наконец, треснула.

Сработала идея "банящих пауз", довольно красивая. Итак, в модели Conditional Sampling, напомню, каждый голос генерирует одну ноту или паузу; каждый из пяти голосов получает на вход то, что сгенерировали предыдущие голоса. Все сгенерированные ноты объединялись (логичиским или), а лосс считался индивидуально для каждой ноты, сравнивая вероятность того, что хотя бы один из голосов сгенерит её, с истиной. 
* при обучении, голоса, получая инфу о том, что предыдущие сгенерировали, скажем, ноту до, могут также сэмплировать до, чтобы увеличить верную вероятность. Технически вроде ничего страшного, логическому или всё равно, сколько раз одну и ту же ноту выдали, но вместо угадывания других нот голос занимается не тем, чем надо.
* в таком виде пятый голос генерировал одну верную ноту, а остальные молчали. Видимо, поскольку при обучении ему приходило "больше" верной информации, он и научился с ней как-то работать.
* работало только со "Священным костылём" (см. описание модели), очень нехорошо.

Важное новое наблюдение:
* если голос генерирует паузу, следующие голоса не получают никакой новой информации и должны решать ту же задачу, что и предыдущий голос.

Казалось бы, ничего плохого, так как все голоса являются разными моделями (разными микролстмами!). Однако именно решение этой проблемы дало плоды. Давайте, пауз будет... тоже 12. И они будут БАНИТЬ одну из нот (то есть выставлять в ячейку окончательный ноль). Тогда каждому последующему голосу будет приходить какая-то информация. 

Тогда каждый голос, фактически, выбирает одну ячейку и выставляет в неё 0 или 1. И обязательно выставляет! Соответственно, с этой ячейкой последующий голоса уже сделать ничего не могут, и вероятности на соответствующих выходах принудительно занялуются. В лоссе вероятность ноты звучать равна вероятности быть засэмплированной * (1 - вероятность быть забаненной).

* работает.
* гармонические зависимости соблюдаются.
* первый голос наиболее активен и всегда сэмплирует первую ноту, последующие уже думают, что делать.
* минус: первый голос почему-то не любит выдавать паузу. Генератор в каждый момент времени выдаёт хотя бы одну ноту; причину пока не понял.

Код: [LSTM real data HistoryUser polyphonic banning.ipynb](https://github.com/FortsAndMills/MusicGeneration/blob/master/RealData%20First%20Model/LSTM%20real%20data%20HistoryUser%20polyphonic%20banning.ipynb)

В данном коде также используется History As Input, где на вход контроллеру дополнительно подаётся история для текущего момента. Фрагменты копирования разредились ещё больше.
...
Код: [LSTM real data HistoryUser polyphonic banning CommonConditionals.ipynb](https://github.com/FortsAndMills/MusicGeneration/blob/master/RealData%20First%20Model/%D0%A1%D0%B2%D0%B0%D0%BB%D0%BA%D0%B0%20%D1%8D%D0%BA%D1%81%D0%BF%D0%B5%D1%80%D0%B8%D0%BC%D0%B5%D0%BD%D1%82%D0%BE%D0%B2/LSTM%20real%20data%20HistoryUser%20polyphonic%20banning%20CommonConditionals.ipynb)

## No aposter

В сэмплах такой стратегии возникло ощущение, что очень редко появляются паузы. Первый голос радостно сэмплирует ноту с почти гарантированной вероятностью --- непонятно. Причина проблемы так и не нашлась, но устранить её удалось.

На вход при обучении следующему голосу подаётся ЛЮБАЯ выбранная предыдущим голосом нота, но с правильным ответом. Если раньше из 24 вариантов "неверные" ответы закрывались и сэмплировался гарантированно правильный ответ из апостериорного распределения, теперь сэмплирование происходит из того распределения, что выдано, и при ошибке "окончательное" решение по выбранной ноте просто инвертируется.

Код: [LSTM real data HistoryUser polyphonic banning CommonConditionals no aposter.ipynb](https://github.com/FortsAndMills/MusicGeneration/blob/master/RealData%20First%20Model/LSTM%20real%20data%20HistoryUser%20polyphonic%20banning%20CommonConditionals%20no%20aposter.ipynb)
...
* заменить представление на 1 = нота зажата вместо 1 = нота зажимается. Придётся дискретизацию увеличить. Дополнительно можно ввести спецсимвол продолжения звучания ноты и требовать от голосов "держать" свои ноты...
